-- Recalculation jobs use the same minimal webhook signal as resume-analysis
-- jobs; the processor still claims the durable row before doing any work.
...
declare
  ...
  webhook_secret text;
begin
  if new.kind::text not in ('resume_analysis', 'recalculate_matches') then
    return new;
  end if;
