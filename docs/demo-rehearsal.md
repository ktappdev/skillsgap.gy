...
1. Open `/login` and choose **Applicant demo**. Start on `/dashboard/overview` for the screenshot-ready pathway snapshot, then open the closest routes and the mechanical-work-to-offshore transfer. Point out the “Curated demo pathway” label and the “Top 3 of 18 roles” count so judges can distinguish rehearsal data from a live vacancy. If using a live CV, pause on **Possible transfers from your CV** and select one taxonomy translation before showing the score; explain that Qwen suggests and the applicant confirms.
2. Open **Trainee Offshore Mechanical Technician**. Explain the deterministic score, BOSIET gate, and hydraulic-maintenance gap. Show the six-week illustrative program, demo intake and cost, then point out that confirming this one requirement projects the match from 82% to 100%.
