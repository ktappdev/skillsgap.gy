...
`I Want to Become` is a public, no-account pathway explorer. A young person chooses a career direction, enters CSEC/CXC subjects and grades or photographs a result slip, and receives an honest, practical route toward that career. Every catalogue occupation explains how the starting point can transfer into Guyana's oil-and-gas ecosystem and provides official places to take the next step.
...
The explorer uses the existing small-win language and visual system, but labels itself as career planning—not a job application. It keeps the catalogue fallback in the browser so the core experience remains useful while Supabase or an external provider is unavailable.
...
The occupation catalogue uses two narrow anonymous RPCs:

- `public.get_public_occupations()` returns active occupation metadata and the reviewed transfer summary.
- `public.get_public_occupation_pathway(occupation_slug text)` returns one occupation, active preparation subjects, and active verified action cards.

The RPCs must not expose company-member data, applicant data, evidence, CV paths, draft roles, unverified action cards, or inactive programs. A narrow projection is preferable to broad anonymous read policies on operational tables.

### Preparation guidance

Add admin-governed migration tables:

```text
career_preparation_subjects
  id, occupation_id, subject_name, minimum_grade (nullable), guidance_note,
  source_url, source_locator, last_verified_at, is_active,
  created_at, updated_at

occupation_pathway_actions
  id, occupation_id, training_program_id (nullable), action_type, title,
  instruction, why_it_helps, organization_name, location, contact_text,
  url, source_url, source_locator, last_verified_at, is_verified, is_active,
  sort_order, created_at, updated_at
```

The `occupations.industry_transfer_summary` field stores the reviewed transfer explanation. The public RPC exposes only active safe fields. Seed only guidance the team can stand behind as preparation, not formal employer requirements. Public action URLs must use HTTPS and are reviewed by an administrator before publication.
...
### Guidance governance and freshness

- The public catalogue is curated, not scraped and not generated by an LLM.
- Each of the 18 occupations has at least three seeded actions: learn, practise, and local-content registration.
- A learner sees the first three active actions as “Your next three moves”; additional verified actions appear below.
- Verification older than 90 days is labelled “Needs checking.” Unverified actions stay out of the public projection.
- When an exact course cannot be verified, the action says to ask the provider about current intake rather than claiming a course is available.
- The admin career-guidance page can edit summaries, add preparation subjects, add or edit action cards, attach existing training programs, verify, and deactivate records.
...
2. Add the safe occupation projections, guidance migration, seed data, static fallback, and admin verification surface.
3. Add unit tests for grade validation, subject aliases, all 18 fallback pathways, safe action URLs, role visibility, and missing-guidance states.
...
- An anonymous visitor can select any of the 18 catalogue occupations and receive a non-empty transfer explanation plus at least three concrete action cards with official links.
...
- If the Supabase guidance migration is pending or unavailable, the static catalogue renders the same validated pathway shape without saving learner data.
- A platform administrator can edit, verify, and deactivate public guidance without changing application code.
...
A school leaver without a CV can anonymously turn either manually entered or photo-read CSEC/CXC results and a selected aspiration into a clear, honest, role-specific action plan with official next-step links, without weakening the existing privacy model or depending on AI availability.
