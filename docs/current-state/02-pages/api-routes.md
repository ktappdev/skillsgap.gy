...
Anonymous. Accepts JSON `{ "text": "…" }` with 10–2000 characters after trimming and otherwise returns 400 `{message}`. Requires `SKILL_PREVIEW_PROCESSOR_URL` plus `SKILL_PREVIEW_PROCESSOR_SECRET`, and returns 503 `{message}` when either is missing.
