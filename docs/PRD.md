...
- The LLM extracts the worker's original wording, work history, qualifications, certifications, education, page evidence, and confidence. For each possible qualification it preserves the original term (for example, `Minibus diesel repair`) and returns one or two `candidate_slugs` from the fresh active taxonomy snapshot. It also suggests the candidate's name, phone, and CV contact email only when clearly presented as the candidate's own; referee, employer, recruiter, and agency details are excluded. Contact values, page evidence, and confidence stay in a private pending suggestion until the applicant chooses to apply them. The sign-in email remains separate and is never replaced. A CV contact email is visible to an employer only after role-specific profile sharing.
...
The working path is image-only: every validated PDF page is rendered and sent to Qwen vision in one ordered request. PP-StructureV3 remains documented as a dormant rollback option, not an active dependency.
...
- [OCR rollback runbook](thundercompute/03-pp-structure-v3-ocr.md)
...
| `apply_resume_extraction_with_contact_details(job_id, extraction)` | Thunder service role only | Persist pending findings/options, employment evidence, and private contact suggestions, then recalculate from confirmed qualifications. |
| `apply_contact_suggestion(target_suggestion_id)` | Authenticated owner | Apply reviewed CV contact values to the applicant's profile. |
| `dismiss_contact_suggestion(target_suggestion_id)` | Authenticated owner | Dismiss a pending CV contact suggestion. |
...
- Do not persist raw rendered page images beyond the processing job. Store structured facts, evidence snippets, and applicant corrections instead.
...
- Implement the Go worker, job claiming, PDF validation/rendering, vision extraction, structured validation, and deterministic matching.
- Build applicant upload, processing state, editable profile, matching, gaps, and training roadmap views.
- Exercise representative clean, scanned, mixed, two-column, and table-heavy fixtures through the full vision pipeline. Record page count, render size, latency, and peak GPU memory.
...
| Risk | Mitigation |
| --- | --- |
| Qwen fails during demo | Use the prepared processed applicant account; show health status and continue with live product data. |
...
| GPU memory pressure | Keep one-worker concurrency, retain the 20 MB rendered-image limit, and lower the model's memory utilization before changing the checkpoint. |
