...
- The LLM extracts the worker's original wording, work history, qualifications, certifications, education, page evidence, and confidence. For each possible qualification it preserves the original term (for example, `Minibus diesel repair`) and returns one or two `candidate_slugs` from the fresh active taxonomy snapshot. It also extracts the candidate's name, phone, and email only when those details are clearly presented as the candidate's own; referee, employer, recruiter, and agency details are excluded. PostgreSQL fills only blank name, phone, and contact-email profile fields. Existing profile values and the account sign-in email are never replaced. A CV contact email is visible to an employer only after role-specific profile sharing.
...
| `apply_resume_extraction_with_contact_details(job_id, extraction)` | Thunder service role only | Persist pending findings/options and employment evidence, fill blank contact fields from the CV, then recalculate from confirmed qualifications. |
