...
function GapAction({ gap, isFirst = false }: { gap: Gap; isFirst?: boolean }) {
  return (
      ...
      {gap.training ? (
        <div className="mt-4">
          ...
          <p className="mt-1 text-sm leading-6 text-muted">
            Ask whether this route currently covers {gap.name.toLowerCase()}. Confirm dates, entry requirements, and cost.
          </p>
...
export function GapActionList({ gaps }: { gaps: Match["gaps"] }) {
  const [firstGap, ...otherGaps] = gaps;
  if (!firstGap) return null;

  return (
    <div className="mt-6 border-y border-border">
      <GapAction gap={firstGap} isFirst />
      {otherGaps.length > 0 ? (
        <details className="border-t border-border py-4">
          ...
          <div className="mt-4">
            {otherGaps.map((gap) => <GapAction key={gap.id ?? `${gap.name}-${gap.type}`} gap={gap} />)}
          </div>
        </details>
      ) : null}
    </div>
