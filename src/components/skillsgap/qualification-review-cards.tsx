  ...
  return (
    <li className="border border-border bg-surface-muted/40 p-4">
      ...
      <div className="mt-4 grid gap-4 border-t border-border pt-4 md:grid-cols-2">
        ...
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end">
          <label className="min-w-0 flex-1 text-xs font-semibold text-muted">Choose a different skill<select value={correction} onChange={(event) => onCorrectionChange(event.target.value)} className="mt-1 min-h-11 w-full min-w-0 border border-border bg-surface px-3 text-base font-normal sm:text-sm"><option value="">Select a skill</option>{availableQualifications.filter((qualification) => qualification.id !== item.qualification_id).map((qualification) => <option key={qualification.id} value={qualification.id}>{qualification.name}</option>)}</select></label>
