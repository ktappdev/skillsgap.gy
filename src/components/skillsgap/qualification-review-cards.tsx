...
export function PendingFindingCard({ finding, availableQualifications, selectedQualificationId, onSelect, onReject }: { finding: ApplicantExtractionFindingView; availableQualifications: Tables<"qualifications">[]; selectedQualificationId: string; onSelect: (qualificationId: string) => void; onReject: () => void }) {
    ...
    <li className="border border-border bg-surface-muted/40 p-4">
      ...
      <div className="mt-4 grid gap-4 border-t border-border pt-4 md:grid-cols-2">
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end">
          ...
          <button type="button" onClick={onSaveYears} className="min-h-11 w-full rounded-md border border-border px-3 text-sm font-semibold text-muted transition-colors hover:border-accent hover:text-accent sm:w-auto">Save years</button>
        </div>
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end">
          ...
          <button type="button" disabled={!correction} onClick={onCorrect} className="min-h-11 w-full rounded-md border border-border px-3 text-sm font-semibold text-muted transition-colors hover:border-accent hover:text-accent disabled:opacity-50 sm:w-auto">Use skill</button>
        </div>
      </div>
    </li>
  );
