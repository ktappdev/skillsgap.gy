...
export function MilestonePath({ demo = false, resumeStatus, processingStatus, hasProfile = false, hasMatches = false, hasEligibleMatch = false }: MilestonePathProps) {
  const liveMilestones = [
    ...
    { title: "Interview slot", detail: hasEligibleMatch ? "Pick a 15-minute slot when invited." : "Meet the match threshold and all required qualifications.", state: hasEligibleMatch ? "complete" : "next" },
  ] as const;
