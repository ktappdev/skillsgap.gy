  ...
  return (
    <section className="mt-6 space-y-6" aria-label="Job fairs and slots">
      <div className="rounded-lg border border-border bg-surface p-5 sm:p-6">
        ...
        <button type="button" onClick={() => { void createFair(); }} className="mt-4 min-h-11 rounded-md bg-accent px-4 text-sm font-semibold text-white hover:bg-accent-strong">Create fair</button>
      ...
      {fairs.length === 0 ? <div className="rounded-lg border border-border bg-surface p-6 text-sm text-muted">No job fairs yet. Plan the first one above.</div> : fairs.map((fair) => (
        <article key={fair.id} className="rounded-lg border border-border bg-surface p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <button type="button" onClick={() => { void toggleFair(fair); }} aria-pressed={fair.status === "open"} className={`min-h-11 rounded-md px-4 text-sm font-semibold ${fair.status === "open" ? "bg-accent text-white hover:bg-accent-strong" : "border border-accent text-accent hover:bg-surface-muted"}`}>{fair.status === "open" ? "Close fair" : "Open fair"}</button>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <button type="button" onClick={() => { void addSlot(fair.id); }} className="min-h-11 rounded-md border border-accent px-4 text-sm font-semibold text-accent hover:bg-surface-muted">Add 15-minute slot</button>
          </div>
        </article>
      ))}
    </section>
