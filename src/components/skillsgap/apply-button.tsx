    ...
    <section className="rounded-lg border border-border bg-surface-muted p-5" aria-live="polite">
      ...
      {canApply ? confirmingWithdraw ? <div className="mt-4 rounded-md border border-red-200 bg-red-50 p-3" role="alert">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" disabled={saving} onClick={() => { void submit(); }} className="min-h-11 rounded-md bg-danger px-4 text-sm font-semibold text-white transition-colors hover:bg-red-700 disabled:cursor-wait disabled:opacity-60">{saving ? "Withdrawing…" : "Withdraw application"}</button>
          <button type="button" disabled={saving} onClick={() => setConfirmingWithdraw(false)} className="min-h-11 rounded-md border border-border bg-surface px-4 text-sm font-semibold text-foreground transition-colors hover:border-accent hover:text-accent disabled:opacity-60">Keep application</button>
        </div>
      </div> : <button type="button" disabled={saving} onClick={handleButtonClick} aria-busy={saving} className={`mt-4 min-h-11 rounded-md px-4 text-sm font-semibold transition-colors disabled:cursor-wait disabled:opacity-60 ${applied ? "border border-danger text-danger hover:bg-red-50" : "bg-accent text-white hover:bg-accent-strong"}`}>{saving ? "Updating…" : applied ? "Withdraw application" : status === "withdrawn" ? "Apply again" : "Apply to this role"}</button> : null}
