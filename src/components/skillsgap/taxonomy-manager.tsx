...
export function TaxonomyManager({ initialItems, initialAliases, qualificationsWithoutTraining }: { initialItems: Qualification[]; initialAliases: Tables<"qualification_aliases">[]; qualificationsWithoutTraining: string[] }) {
  ...
  const qualificationsWithoutTrainingSet = new Set(qualificationsWithoutTraining);
  ...
  async function addQualification() {
    ...
    setMessage(null);
    try {
      const result = await createQualification(name, slug, category, description);
    ...
    <section className="mt-6 space-y-6" aria-label="Qualification taxonomy">
      <div className="rounded-lg border border-border bg-surface p-5 sm:p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="sr-only" htmlFor="new-qualification-description">Qualification description</label><textarea id="new-qualification-description" value={description} onChange={(event) => setDescription(event.target.value)} minLength={10} maxLength={500} rows={3} placeholder="Describe the qualification for employers, applicants, and training providers." className={`${inputClass} py-2 sm:col-span-2`} />
          <button type="button" disabled={savingAction !== null} onClick={() => { void addQualification(); }} className="min-h-11 rounded-md bg-accent px-4 text-sm font-semibold text-white hover:bg-accent-strong disabled:cursor-wait disabled:opacity-60 sm:col-span-2">{savingAction === "qualification" ? "Adding…" : "Add"}</button>
        </div>
      </div>
      ...
      <ul className="space-y-4" aria-label="Qualifications">
        {items.map((item) => {
          ...
          return <li key={item.id} className="rounded-lg border border-border bg-surface p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="font-semibold">{item.name}</p><p className="mt-1 text-xs text-muted">{item.category} · {item.slug}</p>{qualificationsWithoutTrainingSet.has(item.id) ? <p className="mt-2 text-sm font-semibold text-amber-700">No verified active training pathway</p> : null}</div><span className={`inline-flex min-h-11 items-center rounded-md px-2.5 text-xs font-semibold ${draft.is_active ? "bg-accent text-white" : "border border-border text-muted"}`}>{draft.is_active ? "Active" : "Inactive"}</span></div><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-sm font-semibold text-foreground">Canonical name<input value={draft.name} onChange={(event) => updateDraft(item.id, { name: event.target.value })} className={`${inputClass} mt-2 min-h-11`} /></label><label className="text-sm font-semibold text-foreground">Category<select value={draft.category} onChange={(event) => updateDraft(item.id, { category: event.target.value as Category })} className={`${inputClass} mt-2 min-h-11`}><CategoryOptions /></select></label></div><label className="mt-3 block text-sm font-semibold text-foreground">Description<textarea value={draft.description ?? ""} onChange={(event) => updateDraft(item.id, { description: event.target.value })} rows={3} maxLength={500} className={`${inputClass} mt-2 py-2`} placeholder="Explain what this qualification means in plain language." /></label><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><label className="flex min-h-11 items-center gap-2 text-sm text-muted"><input type="checkbox" checked={draft.is_active} onChange={(event) => updateDraft(item.id, { is_active: event.target.checked })} aria-describedby={`${item.id}-active-warning`} className="size-4 accent-accent" /> Include in future CV checks</label><p id={`${item.id}-active-warning`} className="mt-1 max-w-prose text-xs leading-5 text-muted">Unchecking hides this skill from future CV reading and from matching. Applicants who already confirmed it stop being credited for it in new match scores, and requirements that use it can no longer be satisfied.</p></div><button type="button" disabled={savingAction !== null} onClick={() => { void saveQualification(item); }} className="min-h-11 rounded-md border border-accent px-4 text-sm font-semibold text-accent hover:bg-surface-muted disabled:cursor-wait disabled:opacity-60">{savingAction === `qualification:${item.id}` ? "Saving…" : "Save definition"}</button></div>{itemAliases.length > 0 ? <p className="mt-3 text-xs text-muted">Aliases: {itemAliases.map((itemAlias) => itemAlias.alias).join(", ")}</p> : null}</li>;
        })}
      </ul>
    </section>
  );
