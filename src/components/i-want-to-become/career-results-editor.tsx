    ...
    <fieldset>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[minmax(0,1fr)_15rem]">
        <div className="rounded-lg border border-border bg-surface-muted p-4">
          <div className="flex gap-3">
            <div>
              ...
              <p className="mt-1 text-sm leading-5 text-muted">Photo reading may not be available. Manual entry below always works.</p>
            </div>
          </div>
        ...
        <div className="rounded-lg border border-border bg-surface p-4 text-sm leading-6 text-muted">
          <ul className="space-y-1.5">
            ...
            <li>• Your image is sent to SkillsGap.gy and its configured reader for extraction. Only subject and grade suggestions return here; the image is not included in this tab&apos;s pathway draft.</li>
          </ul>
        </div>
      </div>

      {photoPreview ? <figure className="mt-4 flex flex-col gap-3 border border-border bg-white p-3 sm:flex-row sm:items-start"><img src={photoPreview} alt="Selected CSEC/CXC result slip for review" className="max-h-44 w-full border border-border object-contain sm:w-56" /><figcaption className="text-sm leading-6 text-muted">Check that the image is clear, then verify every suggested subject and grade.</figcaption></figure> : null}
      ...
      <div className="mt-6 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">{validResultCount > 0 ? `${validResultCount} ready to review` : "None added yet — you can build without results"}</p>
        </div>
      </div>
      ...
      {hasResultEntries ? <label className={`mt-3 flex gap-3 rounded-lg border border-border bg-surface-muted p-4 ${incompleteResultCount > 0 ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}>
        ...
        <span className="text-sm leading-6 text-foreground"><span className="font-semibold">I reviewed these results.</span> I understand they are planning signals only, not professional qualifications or a job-match score.</span>
      </label> : null}
    </fieldset>
  );
}
