...
- `apply_resume_extraction_with_contact_details(job_id uuid, extraction jsonb)` persists the validated profile and pending taxonomy findings, fills only blank applicant name, phone, and contact-email fields from clearly identified CV details, recalculates matches from already-confirmed qualifications, and marks the job completed in one transaction. Existing profile values and the account sign-in email are never replaced. CV contact email remains private until the applicant shares their profile for a specific role. The database stores model options separately; the model never scores candidates.
