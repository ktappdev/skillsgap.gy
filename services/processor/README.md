...
The processor is the private Thunder Compute coordinator. It accepts a lightweight Supabase webhook, claims durable jobs through Supabase REST RPCs, reads a private CV, extracts text, and persists validated structured facts. It never logs CV text, model input/output, names, email addresses, or phone numbers.
...
- `fail_processing_job(processing_job_id uuid, safe_error_message text)` applies the retry limit and stores only safe applicant-facing error text.
