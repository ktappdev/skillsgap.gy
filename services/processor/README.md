...
`/public/` is reachable over HTTPS with no other network gate, so the shared secret is the only thing separating these routes from the open internet: keep it long and random, hand it to no other client, and set it in one processor environment only. The skill-preview route may share the CSEC secret or use its dedicated override. A missing or wrong secret returns 401, text outside 10–2000 characters returns 400, a burst returns 429 with `Retry-After`, and an unusable taxonomy or model returns 503.
