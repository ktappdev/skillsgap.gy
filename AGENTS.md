...
- Applicants may also describe their work in plain language. That text is an intake source, not a CV: it goes to the same model with the same taxonomy snapshot as untrusted evidence, its findings carry `evidence_method = 'text'` and no page, and the same confirm loop turns selected tags into `applicant_qualifications`. Descriptions never populate employment or contact fields, never weaken the vision-only invariants on CV findings, and allow only one open `description_analysis` job per applicant. Any new processing kind must be added to the `processing_jobs` insert policy and the webhook trigger allowlist.
